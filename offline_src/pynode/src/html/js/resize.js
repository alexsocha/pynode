...
var tlBoundary = { id: boundaryID, x: 0, y: 0, fixed: true, static: true, label: "", topRightLabel: "", topLeftLabel: "", labelStyle: "0,transparent,transparent,True", topRightLabelStyle: "0,transparent,transparent,True", topLeftLabelStyle: "0,transparent,transparent,True", r: 0};
var brBoundary = { id: boundaryID + 1, x: 0, y: 0, fixed: true, static: true, label: "", topRightLabel: "", topLeftLabel: "", labelStyle: "0,transparent,transparent,True", topRightLabelStyle: "0,transparent,transparent,True", topLeftLabelStyle: "0,transparent,transparent,True", r: 0 };
...
function updateLayout() {
	...
	if (greuler_instance !== undefined) {
		...
		if (tlBoundaryIndex === -1 || brBoundaryIndex === -1) {
			greuler_instance.graph.addNode(tlBoundary); greuler_instance.graph.addNode(brBoundary);
			updateLayout();
			return;
		}
		
		greuler_instance.graph.nodes[tlBoundaryIndex].x = 0; greuler_instance.graph.nodes[tlBoundaryIndex].y = 0;
		greuler_instance.graph.nodes[brBoundaryIndex].x = w; greuler_instance.graph.nodes[brBoundaryIndex].y = h;
		
		var constraints = [];
		for (var i = 0; i < greuler_instance.graph.nodes.length; i++) {
			var node = greuler_instance.graph.nodes[i];
			if (node.id === tlBoundary.id || node.id === brBoundary.id || node.static) { continue; }
			if (doConstraints) {
				if (node.x < 0) { node.x = w/4; }
				if (node.x > w) { node.x = (3*w)/4; }
				if (node.y < 0) { node.y = h/4; }
				if (node.y > h) { node.y = (3*h)/4; }
			}
		...
		if (doConstraints) { greuler_instance.options.data.constraints = constraints; }
		else { greuler_instance.options.data.constraints = []; }
		greuler_instance.update();
		if (fixLayoutTimer === undefined) { fixLayoutTimer = setInterval(fixLayout, 4000); }
	}
}

var draggingNode = false;
function fixLayout() {
	if (!doConstraints) { return; }
	if (greuler_instance !== undefined && greuler_instance.graph !== undefined) {
		...
		for (var i = 0; i < greuler_instance.graph.nodes.length; i++) {
			var node = greuler_instance.graph.nodes[i];
			if (node.id === tlBoundary.id || node.id === brBoundary.id || node.static) { continue; }
			if (node.x < 0) { node.x = w/4; didFix = true; }
			if (node.x > w) { node.x = (3*w)/4; didFix = true; }
			if (node.y < 0) { node.y = h/4; didFix = true; }
			if (node.y > h) { node.y = (3*h)/4; didFix = true; }
		}
		if (didFix) { greuler_instance.update(); }
	}
	else if (fixLayoutTimer !== undefined) { clearInterval(fixLayoutTimer); fixLayoutTimer = undefined; }
}
...
function dragNode(dragging, nodeId) {
	if (dragging && !draggingNode) { clickNode(nodeId); }
...
function output_resize() {
	if (greuler_instance !== undefined) {
		...
		if (resizeLayoutTimer === undefined) { resizeLayoutTimer = setTimeout(updateLayout, 100); }
	}
}

function updateContraints(enable) {
	if (enable) { doConstraints = true; updateLayout(); }
	else {
		if (fixLayoutTimer !== undefined) { clearInterval(fixLayoutTimer); fixLayoutTimer = undefined; }
		doConstraints = false; updateLayout();
	}
}
...
function getGraphNodes() {
	var nodes = []
	for (var i = 0; i < greuler_instance.graph.nodes.length; i++) {
		if (greuler_instance.graph.nodes[i].id === tlBoundary.id || greuler_instance.graph.nodes[i].id === brBoundary.id) { continue; }
