...
exports['default'] = function () {
  ...
  function inner(selection) {
    ...
    paths.enter().append('path').attr('stroke', function (d) {
      return d.stroke;
    }).attr('fill', 'transparent').attr('stroke-width', function (d) { return d.lineWidth; }).each(function (d, i) {
	...
	links.selectAll('text').attr('font-size', function (d) { return String(d.weightStyle).split(',')[0] + 'px' });
	links.selectAll('text').attr('fill', function (d) { return String(d.weightStyle).split(',')[1] });
	links.selectAll('text').attr('stroke', function (d) { return String(d.weightStyle).split(',')[2] });
	links.selectAll('text').attr('stroke-width', function (d) { return String(Math.min(Math.floor((parseFloat(String(d.weightStyle).split(',')[0]) / 14.0) * 6.0), 6.0)) + 'px' });
	links.selectAll('text').html(function (d) { return newLineParser(String(d.weight), parseFloat(String(d.weightStyle).split(',')[0])) });
...
exports['default'] = function () {
  ...
  function inner(selection) {
    ...
    // inner label
    g.append('text').classed('label', true).attr('font-family', 'Oswald').attr('text-anchor', 'middle').attr('paint-order', 'stroke').attr('stroke-linecap', 'round').attr('stroke-linejoin', 'round').attr('vector-effect', 'non-scaling-stroke').attr('stroke', function (d) { return String(d.labelStyle).split(',')[2] });
	nodes.selectAll('text.label').attr('font-size', function (d) { return String(d.labelStyle).split(',')[0] + 'px' });
	nodes.selectAll('text.label').attr('fill', function (d) { return String(d.labelStyle).split(',')[1] });
	nodes.selectAll('text.label').each( function (d) { if (String(d.labelStyle).split(',')[3] === 'True') { d3.select(this).attr('stroke', String(d.labelStyle).split(',')[2]); }});
	nodes.selectAll('text.label').attr('stroke-width', function (d) { return String(Math.min(Math.floor((parseFloat(String(d.labelStyle).split(',')[0]) / 14.0) * 6.0), 6.0)) + 'px' });
	nodes.selectAll('text.label').html(function (d) { return newLineParser(String(d.label), parseFloat(String(d.labelStyle).split(',')[0]), true) });
	
    // top-right label
    g.append('text').classed('outer-top-right', true).attr('font-family', 'Oswald').attr('text-anchor', 'start').attr('paint-order', 'stroke').attr('stroke-linecap', 'round').attr('stroke-linejoin', 'round').attr('vector-effect', 'non-scaling-stroke').attr('transform', function (d) { return getLabelPosition(d, 0) });
	nodes.selectAll('text.outer-top-right').attr('font-size', function (d) { return String(d.topRightLabelStyle).split(',')[0] + 'px' });
	nodes.selectAll('text.outer-top-right').attr('fill', function (d) { return String(d.topRightLabelStyle).split(',')[1] });
	nodes.selectAll('text.outer-top-right').attr('stroke', function (d) { return String(d.topRightLabelStyle).split(',')[2] });
	nodes.selectAll('text.outer-top-right').attr('stroke-width', function (d) { return String(Math.min(Math.floor((parseFloat(String(d.topRightLabelStyle).split(',')[0]) / 15.0) * 6.0), 6.0)) + 'px' });
	nodes.selectAll('text.outer-top-right').transition(500).attr('transform', function (d) { return getLabelPosition(d, 0) });
	nodes.selectAll('text.outer-top-right').html(function (d) { return newLineParser(String(d.topRightLabel), parseFloat(String(d.topRightLabelStyle).split(',')[0]), false) });

    // top-left label
    g.append('text').classed('outer-top-left', true).attr('font-family', 'Oswald').attr('text-anchor', 'end').attr('paint-order', 'stroke').attr('stroke-linecap', 'round').attr('stroke-linejoin', 'round').attr('vector-effect', 'non-scaling-stroke').attr('transform', function (d) { return getLabelPosition(d, 1) });
	nodes.selectAll('text.outer-top-left').attr('font-size', function (d) { return String(d.topLeftLabelStyle).split(',')[0] + 'px' });
	nodes.selectAll('text.outer-top-left').attr('fill', function (d) { return String(d.topLeftLabelStyle).split(',')[1] });
	nodes.selectAll('text.outer-top-left').attr('stroke', function (d) { return String(d.topLeftLabelStyle).split(',')[2] });
	nodes.selectAll('text.outer-top-left').attr('stroke-width', function (d) { return String(Math.min(Math.floor((parseFloat(String(d.topLeftLabelStyle).split(',')[0]) / 15.0) * 6.0), 6.0)) + 'px' });
	nodes.selectAll('text.outer-top-left').transition(500).attr('transform', function (d) { return getLabelPosition(d, 1) });
	nodes.selectAll('text.outer-top-left').html(function (d) { return newLineParser(String(d.topLeftLabel), parseFloat(String(d.topLeftLabelStyle).split(',')[0]), false) });
...
var GreulerDefaultTransition = (function (_Graph) {
  ...
  _createClass(GreulerDefaultTransition, [{
    ...
    value: function doTemporalHighlightNode(selection, options) {
      if ('size' in options) {
	  	this.innerNodeSelector(selection).transition('highlight_node_size').duration(this.getAnimationTime() / 2).attr('r', options.size).transition('highlight_node_size').duration(this.getAnimationTime() / 2).attr('r', function(d) { return d.r });
	  }
	  if ('color' in options) {
      	this.innerNodeSelector(selection).transition('highlight_node_color').duration(this.getAnimationTime() / 2).attr('fill', options.color).transition('highlight_node_color').duration(this.getAnimationTime() / 2).attr('fill', function(d) { return d.color });
		
		selection.selectAll('text.label').transition('highlight_node_outline').duration(this.getAnimationTime() / 2).attr('stroke', function(d) { return String(d.labelStyle).split(',')[3] === 'False' ? options.color : String(d.labelStyle).split(',')[2] }).transition('highlight_node_outline').duration(this.getAnimationTime() / 2).attr('stroke', function(d) { return String(d.labelStyle).split(',')[3] === 'False' ? d.color : String(d.labelStyle).split(',')[2] });
	  }
	  return true;
    }
    ...
    value: function doTemporalHighlightEdges(selection, options) {
	  if ('width' in options) {
	  	this.innerEdgeSelector(selection).transition('highlight_edge_width').duration(this.getAnimationTime() / 2).attr('stroke-width', options.width).transition('highlight_edge_width').duration(this.getAnimationTime() / 2).attr('stroke-width', function(d) { return d.lineWidth });
	  }
	  if ('color' in options) {
      	this.innerEdgeSelector(selection).transition('highlight_edge_color').duration(this.getAnimationTime() / 2).attr('stroke', options.color).transition('highlight_edge_color').duration(this.getAnimationTime() / 2).attr('stroke', function(d) { return d.stroke });
	  }
	  return true;
    }
